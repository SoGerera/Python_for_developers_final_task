...
from app.repositories.posts import PostRepository, get_post_reposetory
...
class PostService:
    ...
    async def get_all_posts(
        self, skip: int = 0, limit: int = 100, user_id: Optional[Any] = None, category_id: Optional[Any] = None
    ) -> List[PostResponse]:
        """Получить все посты с фильтрацией"""
        posts = await self.repo.get_all(skip, limit, user_id, category_id)
    ...
    async def create_post(self, post_data: PostCreate, user_id: Any) -> PostResponse:
        """Создать новый пост"""
        post_dict = post_data.model_dump()
        post_dict["user_id"] = user_id
        post_dict.pop("media_id", None)
    ...
    async def search_posts_by_description(
        self, caption_pattern: str, skip: int = 0, limit: int = 100
    ) -> List[PostResponse]:
        """Поиск постов по описанию"""
        posts = await self.repo.search_by_description(
            caption_pattern, skip, limit
        )
        return [post for post in posts]
    ...
    # Статистика
    async def get_posts_count(self, user_id: Optional[Any] = None, category_id: Optional[Any] = None) -> int:
        """Получить общее количество постов с фильтрацией"""
        return await self.repo.get_count(user_id, category_id)
